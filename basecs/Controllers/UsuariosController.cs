...
namespace backend_adm.Controllers
{
    public class UsuariosController : ControllerCS
    {
        ...
        #region CONSTRUCTORS
        public UsuariosController([FromServices] UsuariosService service, [FromServices] LogsService log)
        {
            _service = service;
            _log = log;
        }
            ...
            )
        {
            try
            {
                return Ok(await _service.ReturnListWithParameters(id, grupoUsaruiId, login, nome, email, bloqueado, ativo));
            }
            catch (Exception ex)
            {
                return UnprocessableEntity(ex.Message);
        ...
        #region INSERT
        [HttpPost]
        public async Task<ActionResult<Usuario>> Insert([FromBody] Usuario model)
        {
            try
            {

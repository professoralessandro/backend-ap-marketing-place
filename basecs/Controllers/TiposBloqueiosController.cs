...
{
    public class TiposBloqueiosController : ControllerCS
    {
        #region ATRIBUTTES
        private readonly TiposBloqueiosService _service;
        private readonly LogsService _log;
        #endregion

        #region CONSTRUCTORS
        public TiposBloqueiosController([FromServices] TiposBloqueiosService service, [FromServices] LogsService log)
        {
            _service = service;
            _log = log;
        }
        ...
        [HttpGet]
        public async Task<ActionResult<List<TipoBloqueio>>> ReturnListWithParameters(
            [FromQuery] int? id,
            [FromQuery] string descricao,
            [FromQuery] bool? ativo
            )
        ...
        #region INSERT
        [HttpPost]
        public async Task<ActionResult<TipoBloqueio>> Insert([FromBody] TipoBloqueio model)
        {
            try
            {
        ...
        #region UPDATE
        [HttpPut]
        public async Task<ActionResult<TipoBloqueio>> Update(TipoBloqueio model)
        {
            try
            {

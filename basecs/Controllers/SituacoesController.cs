...
{
    public class SituacoesController : ControllerCS
    {
        #region ATRIBUTTES
        private readonly SituacoesService _service;
        private readonly LogsService _log;
        #endregion

        #region CONSTRUCTORS
        public SituacoesController([FromServices] SituacoesService service, [FromServices] LogsService log)
        {
            _service = service;
            _log = log;
        }
        ...
        [HttpGet]
        public async Task<ActionResult<List<Situacao>>> ReturnListWithParameters(
            [FromQuery] int? id,
            [FromQuery] string descricao,
            [FromQuery] bool? ativo
            )
        ...
        #region INSERT
        [HttpPost]
        public async Task<ActionResult<Situacao>> Insert([FromBody] Situacao model)
        {
            try
            {
        ...
        #region UPDATE
        [HttpPut]
        public async Task<ActionResult<Situacao>> Update(Situacao model)
        {
            try
            {

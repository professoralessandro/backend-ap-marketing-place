...
{
    public class TiposEmailsController : ControllerCS
    {
        #region ATRIBUTTES
        private readonly TiposEmailsService _service;
        private readonly LogsService _log;
        #endregion

        #region CONSTRUCTORS
        public TiposEmailsController([FromServices] TiposEmailsService service, [FromServices] LogsService log)
        {
            _service = service;
            _log = log;
        }
        ...
        [HttpGet]
        public async Task<ActionResult<List<TipoEmail>>> ReturnListWithParameters(
            [FromQuery] int? id,
            [FromQuery] string descricao,
            [FromQuery] bool? ativo
            )
        ...
        #region INSERT
        [HttpPost]
        public async Task<ActionResult<TipoEmail>> Insert([FromBody] TipoEmail model)
        {
            try
            {
        ...
        #region UPDATE
        [HttpPut]
        public async Task<ActionResult<TipoEmail>> Update(TipoEmail model)
        {
            try
            {

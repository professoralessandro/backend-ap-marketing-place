...
{
    public class TiposDocumentosController : ControllerCS
    {
        #region ATRIBUTTES
        private readonly TiposDocumentosService _service;
        private readonly LogsService _log;
        #endregion

        #region CONSTRUCTORS
        public TiposDocumentosController([FromServices] TiposDocumentosService service, [FromServices] LogsService log)
        {
            _service = service;
            _log = log;
        }
        ...
        [HttpGet]
        public async Task<ActionResult<List<TipoDocumento>>> ReturnListWithParameters(
            [FromQuery] int? id,
            [FromQuery] string descricao,
            [FromQuery] bool? ativo
            )
        ...
        #region INSERT
        [HttpPost]
        public async Task<ActionResult<TipoDocumento>> Insert([FromBody] TipoDocumento model)
        {
            try
            {
        ...
        #region UPDATE
        [HttpPut]
        public async Task<ActionResult<TipoDocumento>> Update(TipoDocumento model)
        {
            try
            {

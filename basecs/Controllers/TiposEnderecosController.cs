...
{
    public class TiposEnderecosController : ControllerCS
    {
        #region ATRIBUTTES
        private readonly TiposEnderecosService _service;
        private readonly LogsService _log;
        #endregion

        #region CONSTRUCTORS
        public TiposEnderecosController([FromServices] TiposEnderecosService service, [FromServices] LogsService log)
        {
            _service = service;
            _log = log;
        }
        ...
        [HttpGet]
        public async Task<ActionResult<List<TipoEndereco>>> ReturnListWithParameters(
            [FromQuery] int? id,
            [FromQuery] string descricao,
            [FromQuery] bool? ativo
            )
        ...
        #region INSERT
        [HttpPost]
        public async Task<ActionResult<TipoEndereco>> Insert([FromBody] TipoEndereco model)
        {
            try
            {
        ...
        #region UPDATE
        [HttpPut]
        public async Task<ActionResult<TipoEndereco>> Update(TipoEndereco model)
        {
            try
            {

...
namespace basecs.tests.Services.TiposNotasFiscais
{
    public class TiposNotasFiscaisServiceTest
    {
        #region ATRIBUTTES
        TiposNotasFiscaisService _service;
        Mock<ITiposNotasFiscaisService> _serviceMock;
        #endregion

        #region CONSTRUCTORS
        public TiposNotasFiscaisServiceTest()
        {
            _serviceMock = new Mock<ITiposNotasFiscaisService>();
            _service = new TiposNotasFiscaisService(new Data.MyDbContext());
        }
        #endregion
        ...
        {
            await Assert.ThrowsAsync<Exception>(() => _service.Insert(new basecs.Models.TipoNotaFiscal { TipoNotaFiscalId = 0, Descricao = "D", Ativo = true, UsuarioUltimaAlteracaoId = 1, UsuarioInclusaoId = 1, DataInclusao = DateTime.Now, DataUltimaAlteracao = DateTime.Now }));
        }
        #endregion
        ...
        {
            await Assert.ThrowsAsync<Exception>(() => _service.Update(new basecs.Models.TipoNotaFiscal { TipoNotaFiscalId = 6, Descricao = "D", Ativo = true, UsuarioUltimaAlteracaoId = 1, UsuarioInclusaoId = 1, DataInclusao = DateTime.Now, DataUltimaAlteracao = DateTime.Now }));
        }
        #endregion

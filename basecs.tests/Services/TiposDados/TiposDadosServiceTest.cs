...
namespace basecs.tests.Services.TipoDado
{
    public class TipoDadoServiceTest
    {
        #region ATRIBUTTES
        TiposDadosService _service;
        Mock<ITiposDadosService> _serviceMock;
        #endregion

        #region CONSTRUCTORS
        public TipoDadoServiceTest()
        {
            _serviceMock = new Mock<ITiposDadosService>();
            _service = new TiposDadosService(new Data.MyDbContext());
        }
        #endregion
        ...
        {
            await Assert.ThrowsAsync<Exception>(() => _service.Insert(new basecs.Models.TipoDado { TipoDadoId = 0, Descricao = "D", Ativo = true, UsuarioUltimaAlteracaoId = 1, UsuarioInclusaoId = 1, DataInclusao = DateTime.Now, DataUltimaAlteracao = DateTime.Now }));
        }
        #endregion
        ...
        {
            await Assert.ThrowsAsync<Exception>(() => _service.Update(new basecs.Models.TipoDado { TipoDadoId = 6, Descricao = "D", Ativo = true, UsuarioUltimaAlteracaoId = 1, UsuarioInclusaoId = 1, DataInclusao = DateTime.Now, DataUltimaAlteracao = DateTime.Now }));
        }
        #endregion
